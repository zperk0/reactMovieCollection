...
class NavBar extends React.Component {
    ...
    render() {
        ...
        return (<Navigation logoImgPath="/images/movie.jpg" authenticated={true} menuItems={Pages} router={this.context.router}/>);
    }
}
...
function displayRoutes() {
    ...
    for (let componentName in Pages) {
        if (Pages[componentName].routeParams === true) {
            routesArray.push(<Route key={componentName} path={Pages[componentName].href} exactly component={Pages[componentName].component} />);
        }
        routesArray.push(<Route key={componentName} path={Pages[componentName].href} exactly component={Pages[componentName].component} />);
    }
